<h1 class="page-header">店铺认证审核</h1>
        ...
        <table class="table table-striped">
            ...
            <?php foreach ($data as $k=>$v): ?>
                <tr>
                    ...
                    <td><a href="<?php echo $v->shop_url; ?>" target="_blank"><?php echo $v->shop_name; ?></a></td>
                    <td><?php echo $v->shop_ww; ?></td>
                    <td><?php echo $v->shop_province . $v->shop_city . $v->shop_county . $v->shop_address; ?></td>
                    <td><a href="<?php echo CDN_DOMAIN . $v->shop_pic; ?>" class="fancybox"><img class="item-pic-box" src="<?php echo CDN_DOMAIN . $v->shop_pic; ?>"></a></td>
                    <td><?php echo $v->gmt_create; ?></td>
                    <td>
                        <a href="javascript:;" class="btn btn-sm btn-success btn-approve" data-id="<?php echo $v->id; ?>" data-seller-id="<?php echo $v->seller_id; ?>" data-url="<?php echo base_url('shop_review/operation_handle'); ?>">通过</a>
                        <!--
                        <a href="javascript:;" class="btn btn-sm btn-danger btn-reject" data-id="<?php echo $v->id; ?>" data-url="<?php echo base_url('shop_review/operation_handle'); ?>">拒绝</a>
                        <a class="btn btn-sm btn-danger btn-reject2" href="<?php echo base_url('shop_review/reject?id='. $v->id); ?>">拒绝</a>
                        -->
                        <button class="btn btn-sm btn-danger reject" data-id="<?php echo $v->id?>" data-url="<?php echo base_url('shop_review/reject_handle');?>" data-toggle="modal" data-target="#myModal">拒绝</button>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
