<h1 class="page-header">买手代理商管理</h1>
    ...
    <div class="row" style="margin:10px 20px;">
        <div class="col-md-5">
            <div class="form-group">
                <label>代理商账号: </label>
                <input type="text" name="buyer_name" value="<?php echo $buyer_name?>" placeholder="代理商账号"/>
            </div>
            <div class="form-group">
                <label>授权状态: </label>
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="text-muted bootstrap-admin-box-title"><a href="<?php echo base_url('agent_buyer/add')?>">添加授权代理商</a></div>
            </div>
        </div>
    </div>
</div>
    ...
    <table class="table table-striped">
        ...
        <?php foreach ($data as $v): ?>
            <tr>
                ...
                <td>
                    <a class="btn btn-sm btn-primary" href="<?php echo base_url('agent_buyer/edit?id=' . $v->id); ?>">编辑</a>
                    <?php if ($v->status == \CONSTANT\Agent::STATUS_NORMAL){?>
                        <a class="btn-frozen btn btn-sm btn-primary" data-user-name="<?php echo $v->buyer_name?>" data-act="frozen" data-id="<?php echo $v->id?>" data-url="<?php echo base_url('agent_buyer/frozen')?>" href="javascript:;">暂停授权</a>
                    <?php }else{?>
                        <a class="btn-unfrozen btn btn-sm btn-primary" data-user-name="<?php echo $v->buyer_name?>" data-act="unfrozen" data-id="<?php echo $v->id?>" data-url="<?php echo base_url('agent_buyer/unfrozen')?>" href="javascript:;">解冻授权 </a>
                    <?php }?>
                </td>
            </tr>
        <?php endforeach; ?>
